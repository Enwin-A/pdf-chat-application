# posts.py
"""
This file contains the CRUD operations for the posts table.
"""
...
def test_posts(db: Session = Depends(get_db)):
    post = db.query(models.PDFDocument).all()
    return post
...
def test_posts_sent(post_post: schema.CreatePost, db: Session = Depends(get_db)):
    new_post = models.PDFDocument(**post_post.dict())
    db.add(new_post)
    db.commit()
    db.refresh(new_post)
    return [new_post]
...
def get_test_one_post(id: int, db: Session = Depends(get_db)):
    idv_post = db.query(models.PDFDocument).filter(models.PDFDocument.id == id).first()
    if idv_post is None:
...
def delete_test_post(id: int, db: Session = Depends(get_db)):
    deleted_post = db.query(models.PDFDocument).filter(models.PDFDocument.id == id)
...
def update_test_post(update_post: schema.PostBase, id: int, db: Session = Depends(get_db)):
    updated_post = db.query(models.PDFDocument).filter(models.PDFDocument.id == id)
